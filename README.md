...
### Why Does Make Cuts Generate Another File Instead Of Just Cutting?
By generating a temporary file, it's easy to exclude cuts
or change the ffmpeg command without editing the
cut list or make_cuts script.
...
but the cuts are not exact.
Sometimes you want a cut to be exact.
Also, if the video's compression isn't efficient enough to upload
